...
#' whether to provide just one plausible score (TRUE) or one per booklet(FALSE)
...
plausible_scores = function(dataSrc, parms=NULL, predicate=NULL, items=NULL, 
                            covariates=NULL, keep.observed=TRUE, nPS=1,merge_within_person=FALSE)  
{
  qtpredicate = eval(substitute(quote(predicate)))
  env = caller_env()
  check_dataSrc(dataSrc)
  # to do: items remove factors, general user input function
                             ...
                             merge_within_person=FALSE)
{
  ...
  } else
  {
    ...
    # this seems very strange: items without parameters should give an error methinks
    # removed the semi_join
    # full parms check is done above
    # if the user wishes to supply a parms done on a subset of the dataSrc, he/she can supply such a subset here as well
