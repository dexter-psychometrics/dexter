...
We do this as often as we can afford (currently, three thousand times), and we take the average as an estimate of $P(R_+ \geq c|\theta^*,\mathbf{b}^*)$. Note that $f(\theta|N_+ = s, \mathbf{b}^*)$ is based on those candidates making the target test. There is no assumption of equal populations.
